...
public class ColumnStore {
    ...
    // Dictionary encoding por columna para datos de baja cardinalidad
    private final Map<String, ColumnDictionary> columnDictionaries;
    private final Map<String, Boolean> dictionaryEnabled;
    ...
    // Thread pool para parallel scans
    ...
    public ColumnStore(String tableName, List<Column> schema, PageManager pageManager) {
        ...
        this.dictionaryEnabled = new HashMap<>();
        ...
        for (Column col : schema) {
            ...
            // Inicializar dictionary encoding para columnas string
            if (col.getType() == DataType.STRING) {
                columnDictionaries.put(col.getName(), new ColumnDictionary(col.getName()));
                dictionaryEnabled.put(col.getName(), false); // Se activa automáticamente si hay baja cardinalidad
            } else {
                dictionaryEnabled.put(col.getName(), false);
            }
    ...
    public long insert(Map<String, Object> values) {
        ...
        try {
            ...
            for (String colName : columnNames) {
                ...
                // Aplicar dictionary encoding si está habilitado para columnas string
                Object valueToStore = value;
                if (dictionaryEnabled.getOrDefault(colName, false) && value instanceof String) {
                    ColumnDictionary dict = columnDictionaries.get(colName);
                    if (dict != null) {
                        valueToStore = dict.putOrGet((String) value);
                    }
                }
                ...
                synchronized (blocks) {
                    ...
                    int localPosition = lastBlock.append(valueToStore, columnTypes.get(colName));
                    ...
                    synchronized (zoneMaps) {
                        ...
                        currentZoneMap.update(value); // Usar valor original para zone map
                    }
                    
                    // Evaluar activación de dictionary encoding cada 100 rows
                    if (rowCount % 100 == 0 && !dictionaryEnabled.getOrDefault(colName, false)) {
                        ColumnDictionary dict = columnDictionaries.get(colName);
                        if (dict != null && dict.shouldUseDictionary(rowCount)) {
                            dictionaryEnabled.put(colName, true);
                            System.out.println("Dictionary encoding enabled for column: " + colName);
                        }
                    }
                }
            }
     ...
     * Se ejecuta en background para no bloquear operaciones de escritura.
     */
    private void compactDeletedRows() {
        // Ejecutar compactación en thread separado para no bloquear writes
        Thread compactThread = new Thread(() -> {
            lock.writeLock().lock();
            try {
                System.out.println("Starting background compaction... Deleted rows: " + deletedRows.size());
                
                // Reconstruir rowPositions y columnIndexInverse excluyendo deleted rows
                Map<Long, Map<String, Integer>> newRowPositions = new HashMap<>();
                Map<String, Map<Integer, Long>> newIndexInverse = new HashMap<>();
                Map<String, List<ColumnBlock>> newColumnData = new HashMap<>();
                Map<String, List<ZoneMap>> newZoneMaps = new HashMap<>();
                
                long newRowId = 0;
                for (long oldRowId : rowPositions.keySet()) {
                    if (deletedRows.containsKey(oldRowId)) {
                        continue; // Saltar filas eliminadas
                    }
                    
                    Map<String, Integer> oldPositions = rowPositions.get(oldRowId);
                    Map<String, Integer> newPositions = new HashMap<>();
                    
                    for (String colName : columnNames) {
                        Integer oldPos = oldPositions.get(colName);
                        if (oldPos == null) continue;
                        
                        // Obtener valor original
                        Object value = getValueUnsafe(oldRowId, colName);
                        if (value == null) continue;
                        
                        // Agregar a nuevos bloques
                        List<ColumnBlock> colBlocks = newColumnData.computeIfAbsent(colName, k -> new ArrayList<>());
                        List<ZoneMap> colZoneMaps = newZoneMaps.computeIfAbsent(colName, k -> new ArrayList<>());
                        Map<Integer, Long> colIndexInverse = newIndexInverse.computeIfAbsent(colName, k -> new HashMap<>());
                        
                        ColumnBlock lastBlock = colBlocks.isEmpty() ? null : colBlocks.get(colBlocks.size() - 1);
                        if (lastBlock == null || lastBlock.isFull()) {
                            lastBlock = new ColumnBlock(columnTypes.get(colName), pageManager);
                            colBlocks.add(lastBlock);
                            colZoneMaps.add(new ZoneMap());
                        }
                        
                        int cumulativeSize = 0;
                        for (ColumnBlock block : colBlocks) {
                            if (block != lastBlock) {
                                cumulativeSize += block.size();
                            }
                        }
                        ...
                        newPositions.put(colName, globalPosition);
                        colIndexInverse.put(globalPosition, newRowId);
                        
                        // Actualizar zone map
                        ZoneMap zoneMap = colZoneMaps.get(colZoneMaps.size() - 1);
                        zoneMap.update(value);
                    }
                    
                    newRowPositions.put(newRowId, newPositions);
                    newRowId++;
                }
                
                // Reemplazar estructuras antiguas con las nuevas
                rowPositions.clear();
                rowPositions.putAll(newRowPositions);
                
                for (String colName : columnNames) {
                    columnIndexInverse.get(colName).clear();
                    columnIndexInverse.get(colName).putAll(newIndexInverse.getOrDefault(colName, new HashMap<>()));
                    
                    columnData.get(colName).clear();
                    columnData.get(colName).addAll(newColumnData.getOrDefault(colName, new ArrayList<>()));
                    
                    columnZoneMaps.get(colName).clear();
                    columnZoneMaps.get(colName).addAll(newZoneMaps.getOrDefault(colName, new ArrayList<>()));
                    
                    // Reconstruir offsets acumulados
                    List<ColumnBlock> blocks = columnData.get(colName);
                    int[] newOffsets = new int[blocks.size()];
                    int cumulative = 0;
                ...
                // Limpiar deletedRows
                deletedRows.clear();
                rowCount = (int) newRowId;
                
                System.out.println("Compaction completed. New row count: " + rowCount);
            } catch (Exception e) {
    ...
    /**
     * Obtiene un valor sin adquirir lock (uso interno durante compactación).
     */
    private Object getValueUnsafe(long rowId, String columnName) {
        ...
        int cumulative = blockIndex > 0 ? (offsets.length > blockIndex - 1 ? offsets[blockIndex - 1] : 0) : 0;
    ...
    /**
     * Escanea una columna aplicando un filtro con parallel processing.
     * Retorna los rowIds que cumplen el predicado.
     * Optimizado con índice inverso persistente O(1), zone maps para pruning y parallel scans.
     */
    public List<Long> scanColumn(String columnName, Predicate<Object> predicate) {
        ...
        try {
            ...
            // Usar parallel scan para bloques grandes (> 10 bloques o > 10000 rows totales)
            int totalRows = blocks.stream().mapToInt(ColumnBlock::size).sum();
            boolean useParallel = blocks.size() > 10 || totalRows > 10000;
            
            if (useParallel && SCAN_POOL.getActiveThreadCount() < SCAN_POOL.getParallelism()) {
                // Parallel scan usando ForkJoin
                ScanTask task = new ScanTask(blocks, indexInverse, zoneMaps, predicate, deletedRows, 0, blocks.size());
                return SCAN_POOL.invoke(task);
            } else {
                // Sequential scan para datasets pequeños
    ...
    /**
     * Task recursivo para parallel scan de bloques.
     */
    private static class ScanTask extends RecursiveTask<List<Long>> {
        private final List<ColumnBlock> blocks;
        private final Map<Integer, Long> indexInverse;
        private final List<ZoneMap> zoneMaps;
        private final Predicate<Object> predicate;
        private final Map<Long, Boolean> deletedRows;
        private final int startBlock;
        private final int endBlock;
        private static final int SEQUENTIAL_THRESHOLD = 5; // Usar sequential para <= 5 bloques
        
        ScanTask(List<ColumnBlock> blocks, Map<Integer, Long> indexInverse, 
                 List<ZoneMap> zoneMaps, Predicate<Object> predicate, 
                 Map<Long, Boolean> deletedRows, int startBlock, int endBlock) {
            this.blocks = blocks;
        ...
        protected List<Long> compute() {
            ...
            if (numBlocks <= SEQUENTIAL_THRESHOLD) {
                // Process sequentially
                List<Long> results = new ArrayList<>();
                int cumulative = calculateCumulative(startBlock);
                ...
                // Split task
                int mid = startBlock + numBlocks / 2;
                ScanTask leftTask = new ScanTask(blocks, indexInverse, zoneMaps, predicate, deletedRows, startBlock, mid);
                ScanTask rightTask = new ScanTask(blocks, indexInverse, zoneMaps, predicate, deletedRows, mid, endBlock);
                
                leftTask.fork();
                List<Long> rightResults = rightTask.compute();
                List<Long> leftResults = leftTask.join();
        ...
        private int calculateCumulative(int blockIndex) {
            int cumulative = 0;
